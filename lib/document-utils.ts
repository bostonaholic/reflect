...
export async function generateAndWriteSummary(markdownContent: string, apiKey: string, model: string = 'gpt-4', debug: boolean): Promise<string> {
  console.log(chalk.cyan('Generating summary document...'));
  let summary = await generateSummaryFromContributions(markdownContent, apiKey, model, debug);
...
export async function generateAndWriteBrag(summary: string, apiKey: string, startDate: Date, endDate: Date, model: string = 'gpt-4', debug: boolean): Promise<string> {
  addVisualSpacing();
  console.log(chalk.cyan('Generating brag document...'));
  let brag = await generateBragFromSummary(summary, apiKey, startDate, endDate, model, debug);
...
export async function handleBragGeneration(markdownContent: string, apiKey: string, startDate: Date, endDate: Date, model: string = 'gpt-4', debug: boolean): Promise<string> {
  const summary = await generateAndWriteSummary(markdownContent, apiKey, model, debug);
  return generateAndWriteBrag(summary, apiKey, startDate, endDate, model, debug);
} 